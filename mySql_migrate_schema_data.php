...
try {
    ...
    foreach ($db1Columns as $tableName => $columns) {
        ...
        if (!in_array($tableName, $tablesIgnore)) {
            // check is table in db1 exista in db2
            $sqlToInsert = "SELECT EXISTS (SELECT * FROM information_schema.tables WHERE table_schema = '" . $dbname2 . "' AND table_name = '" . $tableName . "');";
            $stmnt = $bdd2->query($sqlToInsert);
            $tableExists = $stmnt->fetchColumn();

            // checl is table exists and try insert it
            if (!$tableExists) {
                ...
                $bdd2->query($createTableSQL);
                echo "\n\e[1;37;43m table " . $tableName . " created in " . $dbname2 . " db\e[0m\n";
            }

            // handle columns and columns types
            foreach ($columns as $columnName => $columnType) {
                if (!isset($db2Columns[$tableName][$columnName])) {
                    // $addColumn = "ALTER TABLE " . $tableName . " ADD `" . $columnName . "` " . $columnType;

                    // do something here for column
                    ...
                    // do something here for column Types
    ...
    if ($v == 1) {
        if (!empty($catchQueries)) {
            ...
            file_put_contents("queriesCached.sql", $output);
            echo "Queries have been saved to queriesCached.sql\n";
        } else {
